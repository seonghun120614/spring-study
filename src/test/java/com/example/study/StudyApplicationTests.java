...
class StudyApplicationTests {
	...
	/*
	 ...
	 이는 여러 엔티티 인스턴스로 구성되는 퍼시스턴스 컨텍스트이며, 엔티티 인스턴스의 라이프 사이클이
...
//				.contains("Spring Boot gives all the power of the Spring Framework without all of the complexities");
//	}
...
//
//		Iterable<Course> result = queryFactory
//				.selectFrom(course)
//				.where(course.category.eq("Spring"))
//				.fetch();
//
//		assertThat(result).isNotEmpty(); // 예시 검증
//
//		List<Course> result2 = queryFactory
//				.selectFrom(course)
//				.where(course.category.eq("Spring")
...
//
//		/*
//		findAll(OrderSpecifier<?>... orders) 메서드는 QuerydslPredicateExecutor
//		인터페이스에 정의되어 있고,
//		CourseRepository 인터페이스가 QuerydslPredicateExecutor 를 상속 받기 때문에
//		CourseRepository 를 통해 findAll() 을 호출할 수 있다.
//		 */
...
//				.isEqualTo("Getting Started with Spring Cloud Kubernetes");
//	}

//	Criteria API 사용
//	@Test
//	public void givenCoursesCreatedWhenLoadCoursesWithQueryThenExpectCorrectCourseDetails() {
//		// CriteriaBuilder 생성 - Criteria Query 를 정의하는데 사용
//		CriteriaBuilder criteriaBuilder = entityManager.getCriteriaBuilder();
//		// 어떤 타입의 데이터를 조회할 것인지 설정 (SELECT 의 대상)
//		CriteriaQuery<Course> courseCriteriaQuery = criteriaBuilder.createQuery(Course.class);
//		// 쿼리문의 Root 를 정의 - 쿼리의 FROM 절에 해당하는 엔티티를 나타냄 (SELECT * FROM COURSE c 와 같음)
//		Root<Course> courseRoot = courseCriteriaQuery.from(Course.class);
//		// 조회 조건 WHERE 절은 Predicate 타입으로 정의, Spring 인 과정을 조건으로 넣음
...
//
//		assertThat(query.getResultList().size()).isEqualTo(5);
//	}
